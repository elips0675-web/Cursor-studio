...
export default function ProfilePage() {
  ...
  const handleDeletePhoto = (index: number) => {
    if (photos.length <= 1) {
      toast({ variant: "destructive", title: language === 'RU' ? "Ошибка" : "Error" });
      return;
    }
          ...
          <div className="bg-white rounded-[2rem] p-6 app-shadow border border-border/40 mb-6 text-left space-y-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="secondary" className="bg-orange-50 text-orange-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><ZodiacIcon sign={profile.zodiac} /> {profile.zodiac}</Badge>
                {profile.height && <Badge variant="secondary" className="bg-blue-50 text-blue-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><Ruler size={12} /> {profile.height} см</Badge>}
                <Badge variant="secondary" className="bg-primary/5 text-primary border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><Target size={12} /> {profile.datingGoal}</Badge>
                {profile.education && <Badge variant="secondary" className="bg-purple-50 text-purple-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><GraduationCap size={12} /> {profile.education}</Badge>}
                {profile.work && <Badge variant="secondary" className="bg-green-50 text-green-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><Briefcase size={12} /> {profile.work}</Badge>}
                {profile.pets && <Badge variant="secondary" className="bg-amber-50 text-amber-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><Dog size={12} /> {profile.pets}</Badge>}
                {profile.sleepSchedule && (() => { const SleepIcon = getSleepIcon(); return <Badge variant="secondary" className="bg-gray-100 text-gray-600 border-0 gap-1.5 py-2 px-3.5 font-bold text-[10px] rounded-xl shadow-sm"><SleepIcon size={12} /> {profile.sleepSchedule}</Badge>; })()}
              </div>
            </div>
            <div className="h-px bg-border/50"></div>
            <div>
